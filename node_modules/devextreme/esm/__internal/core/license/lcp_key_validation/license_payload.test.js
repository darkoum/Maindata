/**
 * DevExtreme (esm/__internal/core/license/lcp_key_validation/license_payload.test.js)
 * Version: 25.2.7
...
function makeLicense(products, version) {
    ...
    return {
        products: [createProductInfo(v, products)]
    }
}
describe("ProductInfo.isProduct \u2013 product-kind bit flags", () => {
    it.each([{
        name: "DXperienceASP",
        kind: ProductKind.DXperienceASP
    }, {
        name: "DXperienceWPF",
        kind: ProductKind.DXperienceWPF
    }, {
        name: "DXperienceWin",
        kind: ProductKind.DXperienceWin
    }, {
        name: "Blazor",
        kind: ProductKind.Blazor
    }, {
        name: "XAF",
        kind: ProductKind.XAF
    }, {
        name: "DevExtremeHtmlJs",
        kind: ProductKind.DevExtremeHtmlJs
    }, {
        name: "Dashboard",
        kind: ProductKind.Dashboard
    }, {
        name: "Docs",
        kind: ProductKind.Docs
    }, {
        name: "DocsBasic",
        kind: ProductKind.DocsBasic
    }, {
        name: "XtraReports",
        kind: ProductKind.XtraReports
...
describe("LicenseInfo \u2013 payload-level behaviour (analogous to dxvcs LicenseTestHelperTests)", () => {
    ...
    it.each([{
        ...
        name: "DXperienceEnt",
        kind: ProductKind.DXperienceEnt
    }, {
        name: "DXperienceUni",
        kind: ProductKind.DXperienceUni
    ...
    it("findLatestDevExtremeVersion returns the highest matching version", () => {
        const lic = {
            products: [createProductInfo(240, ProductKind.DevExtremeHtmlJs), createProductInfo(250, ProductKind.DevExtremeHtmlJs)]
        };
...
describe("Multi-version license scenarios", () => {
    ...
    it("finds the latest DevExtreme version from multiple product entries", () => {
        const lic = {
            products: [createProductInfo(versionId - 1, ProductKind.DevExtremeHtmlJs), createProductInfo(versionId, ProductKind.DevExtremeHtmlJs)]
        };
        expect(findLatestDevExtremeVersion(lic)).toBe(versionId)
    });
    it("returns older version when newer does not include DevExtreme", () => {
        const lic = {
            products: [createProductInfo(versionId, ProductKind.DXperienceWin), createProductInfo(versionId - 1, ProductKind.DevExtremeHtmlJs)]
        };
        expect(findLatestDevExtremeVersion(lic)).toBe(versionId - 1)
    });
    it("returns undefined when no entry has DevExtremeHtmlJs", () => {
        const lic = {
            products: [createProductInfo(versionId, ProductKind.DXperienceWin), createProductInfo(versionId - 1, ProductKind.Blazor)]
        };
        expect(findLatestDevExtremeVersion(lic)).toBeUndefined()
    })
});
