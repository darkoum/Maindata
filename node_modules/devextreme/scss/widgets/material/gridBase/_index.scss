...
@mixin grid-base($widget-name) {
  @include material-grid-base-cell($widget-name);
      ...
      > td.dx-#{$widget-name}-invalid:not(.dx-field-item-content):not(.dx-validation-pending):not(.dx-#{$widget-name}-select-all) {
        &.dx-command-edit {
          .dx-link:not(.dx-link-icon) {
            margin: 0 $material-command-edit-text-margin;
            min-width: 20px;
          }
    ...
    &.dx-#{$widget-name}-column-chooser-mode-select {
      ...
      .dx-#{$widget-name}-column-chooser-plain, .dx-#{$widget-name}-column-chooser-list {
        .dx-treeview-select-all-item {
          padding-inline-start: $material-grid-base-treeview-select-all-item-offset;
        }
      }
    }
  ...
  .dx-command-ai-header-content {
    max-width: calc(100% - ($material-button-height - $material-grid-base-cell-horizontal-padding + $material-grid-base-command-ai-header-button-offset));

    .dx-#{$widget-name}-text-content {
      padding-left: $material-grid-base-command-ai-text-content-padding;
    }

    .dx-icon-chatsparkleoutline {
      @include dx-icon-sizing($material-grid-base-command-ai-chatsparkleoutline-icon-size);
    }
  }

  .dx-command-ai-header-button {
    left: auto;
    right: $material-grid-base-command-ai-header-button-offset;
  }

  td[style*="text-align: right"] .dx-command-ai-header-button {
    left: $material-grid-base-command-ai-header-button-offset;
    right: auto;
  }
