/**
 * DevExtreme (cjs/__internal/ui/date_box/date_box.base.js)
 * Version: 25.2.7
...
class DateBox extends _m_drop_down_editor.default {
    ...
    _updatePopupHeight() {
        if (this._popup && this._strategy instanceof _m_date_boxStrategy4.default) {
    ...
    _getLongestElementDimensions() {
        ...
        const longestValue = _date.default.format(_date_utils.default.getLongestDate(format, _date.default.getMonthNames(), _date.default.getDayNames()), format);
    ...
    _setSubmitValue() {
        ...
        const submitFormat = _date_utils.default.SUBMIT_FORMATS_MAP[type];
        const submitValue = dateSerializationFormat ? _date_serialization.default.serializeDate(value, dateSerializationFormat) : _date_utils.default.toStandardDateFormat(value, submitFormat);
    ...
    _getDisplayedText(value) {
        ...
        if ("text" === mode) {
            ...
            return _date.default.format(value, displayFormat)
        }
        const format = this._getFormatByMode(mode);
        if (format) {
            return _date.default.format(value, format)
        }
        return _date_utils.default.toStandardDateFormat(value, mode)
    }
    _getFormatByMode(mode) {
        return (0, _support.inputType)(mode) ? null : _date_utils.default.FORMATS_MAP[mode]
    }
    _valueChangeEventHandler(e) {
        ...
        const newValue = _date_utils.default.mergeDates(value, parsedDate, type);
    ...
    _applyInternalValidation(value) {
        ...
        const isDateInRange = isDate && _date2.default.dateInRange(value, this.getDateOption("min"), this.getDateOption("max"), type);
    ...
    _getSerializationFormat() {
        ...
        if (dateSerializationFormat && (0, _config.default)().forceIsoDateParsing) {
    ...
    getDateOption(optionName) {
        ...
        const deserializedDate = _date_serialization.default.deserializeDate(optionValue);
    ...
    _serializeDate(date) {
        ...
        const serializedDate = _date_serialization.default.serializeDate(date, serializationFormat);
