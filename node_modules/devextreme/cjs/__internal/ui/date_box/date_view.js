...
var _date_view_roller = _interopRequireDefault(require("./date_view_roller"));
...
class DateView extends _editor.default {
    ...
    _getDefaultOptions() {
        return Object.assign({}, super._getDefaultOptions(), {
            minDate: _date_utils.default.MIN_DATEVIEW_DEFAULT_DATE,
            maxDate: _date_utils.default.MAX_DATEVIEW_DEFAULT_DATE,
            type: TYPE.date,
            value: new Date,
    ...
    _renderRollers() {
        this._$rollersContainer ?? (this._$rollersContainer = (0, _renderer.default)("<div>").addClass("dx-dateview-rollers"));
        this._$rollersContainer.empty();
        this._createRollerConfigs();
        this._rollers = {};
        Object.keys(this._rollerConfigs).forEach(name => {
            const rollerType = this._rollerConfigs[name].type;
            const $roller = (0, _renderer.default)("<div>").appendTo(this._$rollersContainer).addClass(`dx-dateviewroller-${rollerType}`);
            this._rollers[rollerType] = this._createComponent($roller, _date_view_roller.default, {
    ...
    _createRollerConfigs(type) {
        ...
        this._rollerConfigs = {};
        _date.default.getFormatParts(_date_utils.default.FORMATS_MAP[selectedType]).forEach(partName => {
            this._createRollerConfig(partName)
        })
    }
    _createRollerConfig(componentName) {
        const componentInfo = _date_utils.default.DATE_COMPONENTS_INFO[componentName];
    ...
    _setRollerState(roller, selectedIndex) {
        if (selectedIndex !== roller.selectedIndex) {
            ...
            if (roller.type === ROLLER_TYPE.month) {
                currentDate = Math.min(currentDate, _date_utils.default.getMaxMonthDay(currentValue.getFullYear(), rollerValue))
            } else if (roller.type === ROLLER_TYPE.year) {
                currentDate = Math.min(currentDate, _date_utils.default.getMaxMonthDay(rollerValue, currentValue.getMonth()))
            }
            currentValue.setDate(currentDate);
            currentValue[setValue](rollerValue);
            const normalizedDate = _date2.default.normalizeDate(currentValue, minDate, maxDate);
            currentValue = _date_utils.default.mergeDates(normalizedDate, currentValue, "time");
    ...
    _calculateRollerConfigValueRange(componentName) {
        ...
        if (componentName === ROLLER_TYPE.day) {
            endValue = _date_utils.default.getMaxMonthDay(curDate.getFullYear(), curDate.getMonth());
            if (minYear && minMonth) {
