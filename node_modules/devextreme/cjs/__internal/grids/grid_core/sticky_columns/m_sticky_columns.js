...
var _utils = require("../keyboard_navigation/utils");
var _m_utils = _interopRequireDefault(require("../m_utils"));
var _const2 = require("../master_detail/const");
var _utils2 = require("../master_detail/utils");
...
const baseStickyColumns = Base => class extends Base {
    _addStickyColumnBorderLeftClass($cell, column, rowIndex) {
        ...
        const isFirstFixedCell = (0, _utils3.isFirstFixedColumn)(this._columnsController, column, rowIndex, onlyWithinBandColumn, fixedPosition);
        if (isFirstFixedCell) {
    ...
    _addStickyColumnBorderRightClass($cell, column, rowIndex) {
        let onlyWithinBandColumn = arguments.length > 3 && void 0 !== arguments[3] ? arguments[3] : false;
        let fixedPosition = arguments.length > 4 ? arguments[4] : void 0;
        const isLastFixedCell = (0, _utils3.isLastFixedColumn)(this._columnsController, column, rowIndex, onlyWithinBandColumn, fixedPosition);
        if (isLastFixedCell) {
    ...
    toggleColumnNoBorderClass($cell, column, rowIndex) {
        ...
        const needToRemoveBorder = (0, _utils3.needToRemoveColumnBorder)(columnsController, column, rowIndex, isRowsView);
    ...
    updateColumnNoBorderClasses() {
        ...
        $rows.forEach((row, index) => {
            ...
            columns = (0, _utils3.processFixedColumns)(this._columnsController, columns);
    ...
    _createCell(options) {
        ...
        if (hasStickyColumns && !(0, _utils3.needToDisableStickyColumn)(this._columnsController, column)) {
            this.toggleColumnNoBorderClass($cell, column, rowIndex);
            if (column.fixed) {
                const fixedPosition = (0, _utils3.getColumnFixedPosition)(this._columnsController, column);
    ...
    setStickyOffsets(rowIndex, offsets) {
        ...
        columns.forEach((column, columnIndex) => {
            if (column.fixed) {
                ...
                const offset = (0, _utils3.getStickyOffset)(columnsController, columns, widths, columnIndex, offsets);
                if (offsets) {
                    offsets[column.index] = offset
                }
                const styleProps = (0, _utils3.normalizeOffset)(offset);
    ...
    _resizeCore() {
        ...
        const hasStickyColumns = this.hasStickyColumns();
        const hasHidingColumnsQueue = !!(null !== (_this$_adaptiveColumn = this._adaptiveColumnsController) && void 0 !== _this$_adaptiveColumn && null !== (_this$_adaptiveColumn = _this$_adaptiveColumn.getHidingColumnsQueue()) && void 0 !== _this$_adaptiveColumn && _this$_adaptiveColumn.length);
...
const rowsView = Base => class extends(baseStickyColumns(Base)) {
    ...
    setStickyOffsetsForGroupCells() {
        ...
        groupColumns.forEach(column => {
            ...
            const visibleColumnIndex = rtlEnabled ? columns.length - columnIndex - 1 : columnIndex;
            const offset = (0, _utils3.getStickyOffset)(this._columnsController, columns, widths, visibleColumnIndex);
            const styleProps = (0, _utils3.normalizeOffset)(offset);
    ...
    _renderCellContent($cell, options, renderOptions) {
        ...
        const isDetailRowResult = (0, _utils2.isDetailRow)(options);
...
const columnsResizer = Base => class extends Base {
    ...
    _correctColumnIndexForPoint(point, correctionValue, columns) {
        ...
        if (isWidgetResizingMode && !(0, _utils3.isFixedEdge)(point, column, nextColumn)) {
            const $container = (0, _renderer.default)(this._columnHeadersView.getContent());
...
const draggingHeader = Base => class extends Base {
    _generatePointsByColumns(options) {
        ...
        if (hasStickyColumns && isDraggingBetweenHeaders) {
            const columnFixedPosition = (0, _utils3.getColumnFixedPosition)(this._columnsController, sourceColumn);
            switch (true) {
...
const editorFactory = Base => class extends Base {
    ...
    updateFocusOverlay($element) {
        ...
        if (!isHideBorder) {
            ...
            const isGroupElement = (0, _utils.isGroupRow)($element);
            const isGroupFooterRowElement = (0, _utils.isGroupFooterRow)($element);
            const isAdaptiveElement = (0, _utils.isAdaptiveItem)($element);
